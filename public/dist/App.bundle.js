    ...
    _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
        ...
        markers.forEach(function (marker) {
            return marker.addListener('click', function () {
                var html = '\n                <div class="popup">\n                    <a href="/store/' + this.place.slug + '">\n                       <img src="/uploads/' + (this.place.photo || 'store.png') + '" alt=' + this.place.name + ' />\n                       <p>' + this.place.name + ' - ' + this.place.description + '</p>\n                    </a>\n                </div>\n            ';
...
function typeAhead(search) {
    ...
    searchInput.on('input', function () {
        ...
        _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
            ...
            searchResults.innerHTML = '\n                    <div class="search__result">No results for ' + _dompurify2.default.sanitize(_this.value) + '</div>\n                ';
        }).catch(function (err) {
            console.error(err);
        });
    });
    ...
    searchInput.on('keyup', function (e) {
        ...
        var next = void 0;
        if (current) {
            if (e.keyCode === 40) next = current.nextElementSibling ? current.nextElementSibling : items[0];else if (e.keyCode === 38) next = current.previousElementSibling ? current.previousElementSibling : items[items.length - 1];else if (e.keyCode === 13) {
                window.location = current.href;
                return;
            }
        } else {
            // nocurrent
            if (e.keyCode === 40) next = items[0];else if (e.keyCode === 38) next = items[items.length - 1];
        }
