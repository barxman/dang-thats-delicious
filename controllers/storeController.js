...
const confirmOwner = (store, user) => {
  if (!(user && store.author.equals(user._id))) { // store.author is a _id so use equals
...
exports.searchStores = async (req, res) => {
  const stores = await Store
    ...
    }, { // project additional fields - in this case the matching score
      score: { $meta: 'textScore' }
    })
    .sort({
      score: { $meta: 'textScore' } //check - do we need to say meta again?
    })
    .limit(5);
...
exports.heartStore = async (req, res) => {
  ...
  const operator = hearts.includes(req.params.id) ? '$pull' : '$addToSet'; // not push which might add twice
